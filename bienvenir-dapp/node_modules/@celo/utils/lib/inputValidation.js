...
function validateDecimal(input, lng) {
    // Comma decimal points -- only supports es right now
    var decimalChar = lng && lng.startsWith('es') ? ',' : '.';
    var regex = decimalChar === ',' ? /[^0-9,]/g : /[^0-9.]/g;
    var cleanedArray = input.replace(regex, '').split(decimalChar);
